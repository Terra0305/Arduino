...
안 되면 **🙋 선생님, 안 돼요** → 이름 + 질문 글 + 코드 붙여넣기 → 보내기.
보내면 답변 대기 화면으로 바로 이동하고, 선생님이 답변하면 화면에 자동으로 나타납니다.
같은 브라우저에서 여러 번 보내면 **내 질문 목록**에 최신순으로 모이며, 각 코드와 답변이 한 묶음으로 보입니다. 긴 코드는 누를 때만 펼쳐집니다.
다른 노트북이나 며칠 뒤에는 **📬 질문 게시판**에서 학생이 쓴 글과 선생님 답변을 확인할 수 있습니다. 첨부한 코드는 공개되지 않습니다.
...
1. **새 수업 만들기** — 제목, 설명, 제목을 붙인 Arduino 코드 여러 개, 준비물(한 줄에 하나), 연결 방법(글/그림), 한 줄 공지
...
1~4단계 코드를 **한 번에 다 올려두고**, 학생에게는 한 단계씩 보여줄 수 있습니다.

1. 수업을 만들 때 코드마다 있는 **처음부터 학생에게 공개** 체크를 1단계만 켜 둡니다
   (코드를 추가하면 그다음부터는 자동으로 꺼진 채로 시작합니다)
2. 수업 중에는 관리자 메인 **코드 공개**에서 **▶ 다음 단계 공개**를 누릅니다
   (**모두 공개**, 코드별 **공개하기 / 숨기기**, **처음 상태로** 도 있습니다)
3. 학생 화면은 10초마다 스스로 확인해서, F5 를 누르지 않아도 새 코드가 나타납니다

숨긴 코드는 제목도 내용도 학생 화면으로 아예 보내지 않으므로, 페이지 소스를 봐도 미리 알 수 없습니다.
