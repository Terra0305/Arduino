...
1. **빈칸 문제 자동 생성 — ✅ 구현됨.** 완성된(정답) 코드를 주면 `scripts/make-blanks.mjs`가
   ...
   반영하는 흐름으로 쓴다.
2. **수업 복제(Duplicate).** 새 수업을 만들 때마다 준비물·공지·연결 방법 그림을 매번 새로 입력하는
   대신, 기존 수업을 복제해서 코드만 바꿔 새 수업으로 저장.
3. **스냅샷 / 복원 CLI.** `admin-cli.mjs`에 `snapshot <classId>`(현재 상태를 로컬 파일로 저장) /
   `restore <classId> <파일>`(되돌리기) 명령 추가. 수업 중 실수로 잘못된 코드를 올렸을 때 바로 복구.
4. **(선택) 요일/시간 예약 자동 전환.** "매주 화요일 3시엔 자동으로 5번 수업으로" 같은 스케줄링.
   Vercel Cron 등 별도 스케줄러가 필요해 앞의 항목들보다 구현 비용이 크다.
