...
관리자 화면을 사람이 클릭하는 대신, 같은 요청을 코드로 보내는 CLI가 갖춰져 있다.
**사용자가 수업 내용(제목 / 단계별 코드 / 공개 여부)을 텍스트로 주면, 이 도구로 실제 사이트에 바로 반영한다.**

- 프로젝트 루트의 `.env`에 `ADMIN_SITE_URL`과 `ADMIN_PASSWORD`가 이미 들어 있다 — **매번 사용자에게 다시 물어보지 말고 그대로 쓴다.** (`.env`는 `.gitignore`에 있어 커밋되지 않는다.)
...
- `.env`의 `ADMIN_PASSWORD`는 **실제 배포된 사이트의 관리자 비밀번호**다. 채팅이나 로그에 그대로 출력하지 않는다.
- 로그인이 401로 실패하면 Vercel에서 비밀번호가 바뀐 것이다 — 사용자에게 실제 값을 다시 물어보고 `.env`를 갱신한다.
- 이 CLI가 만들거나 바꾸는 내용은 학생 화면에 바로 반영된다. 사용자가 구체적으로 요청한 내용만 그대로 반영하고, 내용이 불분명하거나 기존 수업을 덮어쓰는 경우처럼 판단이 필요한 상황은 평소처럼 확인한다.
- 수업 삭제처럼 되돌리기 어려운 동작은 이 CLI에 아예 넣지 않았다 — 필요하면 관리자 화면에서 직접 하도록 안내한다.
