...
수업 준비를 더 편하게 만들기 위한 아이디어. 우선순위 순.

1. **빈칸 문제 자동 생성 — ✅ 구현됨.** 완성된(정답) 코드를 주면 `scripts/make-blanks.mjs`가
   `pinMode` / `digitalWrite` / `digitalRead` / `delay()` / 비교문(`== HIGH`) 안의 학습 포인트를
   자동으로 `____`로 뚫는다. 난이도 3단계(`value` → `pin` → `mode`, 인자 없이 실행하면 한 번에 전부
   출력)를 지원하고, 정답은 코드에 실제로 나타나는 순서(왼쪽→오른쪽, 위→아래) 그대로 나온다.
...
- **1단계는 항상 빈칸 없이 전체 공개**(`released: true`)로 올린다 — 워밍업 겸 "오늘 이런 걸 만든다"를
  보여주는 초안이라 학생이 바로 볼 수 있어야 한다.
- **2단계부터는 항상 빈칸(`____`)이 있어야 한다.** 사용자가 직접 빈칸을 뚫어서 코드를 줬으면 그대로
  쓰고, 완성 코드만 줬으면 `scripts/make-blanks.mjs`로 만든다. 이유를 들어 빈칸을 빼는 임의 판단은
  하지 않는다 — 애매하면 사용자에게 먼저 확인한다.
- **2단계 이후는 전부 `released: false`로 올린다.** 수업 시작할 때 자동으로 순서대로 공개되는 게
  아니라, 수업 진행 중에 사용자가 직접 `/admin` 화면(또는 `release-next` 명령)으로 한 단계씩 공개한다.
  코드를 만들거나 사이트에 올리는 것과, 그걸 공개 타이밍에 맞춰 여는 것은 별개의 행동이다.
